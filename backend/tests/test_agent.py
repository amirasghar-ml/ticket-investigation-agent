...
def test_seed_and_login_logs():
    seed()
    result = DatabaseTool().search_logs(
        keywords=["login"], status_code=500, hours=48, path="/api/auth/login"
    )
...
def test_full_investigation():
    ...
    assert "jane.doe@acme.com" in report["root_cause"]
    assert "legacy_refresh" in report["root_cause"]
    assert "omar.hassan@acme.com" not in report["root_cause"]
    assert "session_type=None" not in report["root_cause"]
